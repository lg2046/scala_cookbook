...
object Wallet {
  def props(shopperId: Long, cash: BigDecimal) = Props(new Wallet(shopperId, cash))
...
class Wallet(shopperId: Long, cash: BigDecimal) extends PersistentActor with ActorLogging {
