package dp.d3_bahavioral.d8_resp_chain
...
object PartialFunctionDispenser {
  //一个函数就是一个handler
  def dispense(dispenserAmount: Int): PartialFunction[(Money, ArrayBuffer[String]), (Money, ArrayBuffer[String])] = {
    case (Money(amount), result) if amount >= dispenserAmount =>
      val notes = amount / dispenserAmount
      val left = amount % dispenserAmount
      result.append(s"""取 $notes 张 $dispenserAmount.""")
      (Money(left), result)
    case arg@(Money(_), _) => arg
  }
}
...
object PartialDispenseDemo {
  def main(args: Array[String]): Unit = {
    ...
    val handles = dispenser(50)
      .andThen(dispenser(20))
      .andThen(dispenser(10))
      .andThen(dispenser(5))
      .andThen(dispenser(1))

    val result = new ArrayBuffer[String]
    handles(Money(100), result)
